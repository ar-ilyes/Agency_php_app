            ...
            <?php else: ?>
                <?php foreach ($partners as $partner): ?>
                    <div class="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-300">
                        <div class="p-6">
                            <h2 class="text-xl font-semibold mb-2 text-gray-800"><?= htmlspecialchars($partner['nom']) ?></h2>
                            <div class="space-y-2 mb-4">
                                <p class="text-gray-600">
                                    <span class="font-medium">Catégorie:</span> 
                                    <?= htmlspecialchars($partner['categorie']) ?>
                                </p>
                                <p class="text-gray-600">
                                    <span class="font-medium">Ville:</span> 
                                    <?= htmlspecialchars($partner['ville']) ?>
                                </p>
                                <p class="text-gray-600">
                                    <span class="font-medium">Remise:</span> 
                                    <span class="text-green-600 font-semibold"><?= htmlspecialchars($partner['remise']) ?>%</span>
                                </p>
                            </div>
                            <a href="details-partenaire.php?id=<?= $partner['id'] ?>" 
